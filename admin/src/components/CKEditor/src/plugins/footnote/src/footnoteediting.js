...
export default class FootNoteEditing extends Plugin {
  ...
  _deleteModify() {
    ...
    this.listenTo(
      ...
      (evt, data) => {
        ...
        if (
          (doc.selection.anchor.offset === 0 && positionParent.maxOffset === 1) ||
  ...
  _defineConverters() {
    ...
    conversion.for('dataDowncast').elementToElement({
      model: 'footNoteTitle',
      view: (modelElement, { writer: viewWriter }) => createTitleView(modelElement, viewWriter)
    })
    ...
    conversion.for('dataDowncast').elementToElement({
      model: 'footNoteItem',
      view: (modelElement, { writer: viewWriter }) => createItemView(modelElement, viewWriter)
    })
    ...
    function createItemView(modelElement, viewWriter) {
      ...
      const itemView = viewWriter.createContainerElement('a', {
        class: 'footnote-item',
        href: `#${id}`
      })
    ...
    function createPlaceholderView(modelElement, viewWriter) {
      ...
      const placeholderView = viewWriter.createContainerElement('a', {
        class: 'noteholder',
        href: `#${id}`
      })
